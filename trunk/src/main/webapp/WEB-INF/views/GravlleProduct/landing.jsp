...
<div class="tab-pane active" id="tab1">
    ...
    <div class="form-group">
        ...
        <div class="form-group" id="no_search_result_action" style="display: none;">
            <div class="row">
                <div class="col-md-12 text-center">
                    <h4 class="block">Looks like we couldn't find any matching products in your channels. </h4>
                    <h4>Continue to categories page to select the category under which your product will reside.</h4>
...
<div class="tab-pane" id="tab2">
    <h3 class="block">Select a Category to assign to your Product.</h3>

    <div class="form-group">
        <div id="columns" data-selected-category="" class="column-view-container center-block"></div>
...
<script src="${pageContext.request.contextPath}/resources/js/core/gravlle-core.js"></script>
...
var SearchList = function () {
    return {
        ...
        buildSearchProductResultDataTable: function (product) {
            ...
            if (productSearchObject) {
                ...
                for (var key in productSearchObject) {
                    ...
                    productHtml += '<tr class="gradeX" role="row">' +
                            ...
                            '</td>' +
                            '<td>' +
                            oneProduct['attributes.binding'] +
                            '</td>' +
                            '<td>' +
                            '<a href="javascript:;" data-category-name="' +
                            oneProduct['attributes.binding'] +
                            '" class="copy-product btn green button-submit">' +
    ...
    return {
        ...
        init: function () {
            ...
            $('#search_product').bind("click", function () {
                ...
                if (product && product != '{}') {
                    foundProducts = true;
                    SearchList.buildSearchProductResultDataTable(product);

                    CopyProduct.init();

                    try {
                        this.searchResultJson = jQuery.parseJSON(product);
                    } catch (e) {
                        var loggerOptions = {
                        ...
                        GRAVLLE_ERP.gravlleLogger(loggerOptions);
                    }
                } else {
...
var CopyProduct = function () {
    return {
        init: function () {
            $('.copy-product').bind("click", function () {

                var selectedCategory = $(this).attr('data-category-name');
                var categoryId = GRAVLLE_CATEGORY_TREE.getCategoryIdByName({categoryNameToSearch: selectedCategory});
                $('.column-view-container').attr('data-selected-category', categoryId);
...
var CategoryTreeObj = function () {
    return {
        initCategorySelectionTree: function () {

            var categoryId = $('.column-view-container').attr('data-selected-category');
            categoryId = categoryId || undefined;

            $('body').on('click', 'a.assign-category', function() {
                $('#next_product_detail_page').click();
                GRAVLLE_ERP.gravlleLogger({message: "Assigning selected category", type: "info"})
            });
...
var OpenMarketProductSearchWizardObj = function() {
    return {
        initProductSearchScreen: function () {
            var foundProducts = false;
            $('#search_product_container').hide();
            $('#no_search_result_action').hide();
            ...
            $('body').on("click", '#search_product', function () {
                ...
                if (product && product != '{}') {
                    foundProducts = true;
                    // TODO: remove this by adding this logic in service layer
                    product = GRAVLLE_PRODUCT_WIZARD.temp_processProdctJson(product);

                    OpenMarketProductSearchWizardObj.buildSearchProductResultDataTable(product);

                    OpenMarketProductSearchWizardObj.copySelectedProductFromSearchResult();

                    try {
                        window.searchResultJson = product;
                    } catch (e) {
                        var loggerOptions = {
                            message: "Product search with search string: " + searchString + " was unsuccessful",
                            type: "info",
                            username: GRAVLLE_ERP.currentUserLoginVersion()
                        };
                        GRAVLLE_ERP.gravlleLogger(loggerOptions);
        ...
        buildSearchProductResultDataTable: function (product) {
            ...
            if (productSearchObject) {
                ...
                var productSet = JSON.parse(jsonSearchResult);
                for (var key in productSearchObject) {
                    var oneProduct = productSearchObject[key];
                    productHtml += '<tr class="gradeX" role="row">' +
                            '<td>' +
                            key +
                            '</td>' +
        ...
        copySelectedProductFromSearchResult: function () {
            $('body').on('click', '.copy-product', function () {
                ...
                var searchJson = window.searchResultJson;
                var option = {searchKey:"id",searchJson: searchJson};
                window.selectedProductObj = GRAVLLE_ERP.getJavascriptObjectFromJsonByKey(option);

                CategoryTreeObj.initCategorySelectionTree();
                $('#next_product_detail_page').click();
            });
        }
    };
...
OpenMarketProductSearchWizardObj.initProductSearchScreen();
